...
canopy.CenterX = 0;%cm
canopy.CenterY = 0;%cm
canopy.CenterZ = 30;%cm
canopy.AngleX = 0;%degree
canopy.AngleY = 0;%degree

canopy.InitialShadowEdgeLen = 50;%cm

light.R = 200;%cm
light.Elevation = 45;%degree
...
index=1;
opt=0;
for z_values=30:5:45
    %update angle limits for depending on z angles. Do not forget that
    %since we can tilt in 2 axis it is hard to calculate lets make it completly brute force, if it is not in range ignore the solution.    
    canopy.CenterZ = z_values;
    
    for xangle=-7.5:2.5:7.5
        canopy.AngleX = xangle;
        
        for yangle=-7.5:2.5:7.5
...
initialShadowEdge = CanopyValues.InitialShadowEdgeLen;
...
initialArea = CanopyValues.InitialShadowEdgeLen^2;
...
function validation = IsValidSolution(PoleEnds)
    validation_t = 1;
    for i=1:4
        t = sprintf('C%d',i);
        z = getfield(PoleEnds,t)*[0;0;1];
        if(z<30 || z>50)
            validation_t=0;
        end
        if(validation_t == 0)
